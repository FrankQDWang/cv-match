...
async def run_controller_stage(
    *,
    ...
    return controller_decision, {
        ...
        "started_at": controller_started_at,
        "started_clock": controller_started_clock,
    }


def complete_controller_stage(
    *,
    settings: AppSettings,
    controller: object,
    controller_decision: ControllerDecision,
    controller_stage_state: dict[str, Any],
    round_no: int,
    tracer: RunTracer,
    progress_callback: ProgressCallback | None,
    build_llm_call_snapshot: BuildSnapshot,
    ...
    latency_ms = max(1, int((perf_counter() - controller_stage_state["started_clock"]) * 1000))
    controller_provider_usage = getattr(controller, "last_provider_usage", None)
    tracer.write_json(
        f"rounds/round_{round_no:02d}/controller_call.json",
        build_llm_call_snapshot(
