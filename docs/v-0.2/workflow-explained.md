...
- `pinned_filters`：必须保留的筛选条件（地点一旦确认就钉在这里）
...
如果返回的简历去重后数量不够（比如很多是之前轮次已经见过的），Runtime 会在**同一轮内翻页补拉**——但只允许翻页，不允许改搜索词。搜索词的变化只能发生在下一轮。
...
关键理解：反思顾问只是**提建议**，不做最终决定，也不能直接结束 run。最终决定权在 Controller，runtime 只执行 Controller 或硬终止规则。
