...
| Phase 2.1.1: Completed-Round Stop Fix | In progress | 已发现 `min_rounds` 应按已完成检索轮数判断；修复后需要 replay 验证。 |
...
Phase 2.1.1 当前发现：0.4.4 replay 暴露出 `min_rounds` 的语义歧义。原实现按 pending controller round 判断，例如 controller round 3 可以在只完成 2 个检索轮后 stop。业务语义应是“至少完成 3 个检索轮”，所以 stop guidance 需要暴露 `retrieval_rounds_completed`，并按已完成检索轮数判断是否允许停止。该修复完成后应以新版本 replay 验证，不能用 0.4.4 结果推断修复效果。
...
0.4.4 的平均 rounds 为 3.00，这本身不异常。合理的 3 轮 stop 应该表示：核心 query family 已覆盖、top pool 稳定、继续搜索边际收益低。真正的问题不是“轮次太少”，而是低分样本也可能因为内部 top pool 被判为 `usable` 而早停。后续 stop policy 不能只看 productive rounds 和 `usable` pool，还要看强匹配数量、风险、未尝试 high-signal family，以及最终 precision 风险。
...
### Phase 2.1.1: Completed-Round Stop Semantics and Stop Quality Gate

状态：进行中。先修复 `min_rounds` 语义，再决定是否扩大 stop quality gate。

目标：让“3-10 轮”的业务约束按已完成检索轮数生效，并避免低质量 `usable` top pool 过早结束搜索。
...
- 修复后 bump 新版本并重新跑 6 条真实 CTS + 真实 LLM replay。
- 如果低分样本仍早停，再小范围增加 stop quality gate，例如 strong-fit count、high-risk fit count、untried high-signal family。

验收：

- 不再出现完成少于 3 个检索轮就 controller stop 的 run。
- `agent_jd_005` 的停止原因能解释是 search-space exhaustion、alias gap，还是 quality gate 仍偏松。
...
当前下一步建议：完成 `Phase 2.1.1` 的 min-round semantics 修复验证，并用新版本 replay 重新评估 `agent_jd_005`、`agent_jd_004`、`agent_jd_006` 的 stop quality。之后再进入 Phase 2.2 benchmark expansion，不要先做 schema slimming 或 reasoning model A/B。
